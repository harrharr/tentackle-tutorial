...
import com.example.tracker.pdo.md.persist.UserPersistence;
import com.example.tracker.persist.PersistenceBundle;
import com.example.tracker.persist.md.trip.UserRemoteDelegate;
...
public class UserPersistenceImpl extends OrgUnitPersistenceImpl<User, UserPersistenceImpl> implements UserPersistence {
  ...
  /** true if user is allowed to change its own password. */
  private boolean passwordChangeable;

  /** true if user is allowed to maintain its own preferences. */
  private boolean changingPreferencesAllowed;

  /** true if user is restricted to system preferences only. */
  ...
  private static final List<Integer> VALID_CLASS_IDS = List.of(1001);
  ...
  // component list of User2Group nmLinks via User2Group#userId
  ...
  /**
   * Gets the relation nmLinks without performing a database query if not loaded.
   *
   * @return the nmLinks
   */
  ...
  /**
   * Updates the components in the snapshot object.<br>
   ...
   * This will encrypt the TRIP calls below.
   */
