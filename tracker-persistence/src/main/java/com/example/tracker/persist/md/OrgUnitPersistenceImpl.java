...
import com.example.tracker.persist.md.trip.OrgUnitRemoteDelegate;
...
public class OrgUnitPersistenceImpl<T extends OrgUnit<T>, P extends OrgUnitPersistenceImpl<T, P>>
  ...
  static {
    CLASSVARIABLES.setForeignReferencesInitializer(() -> {
      @SuppressWarnings("unchecked")
      AbstractPersistentObject<?,?> po = (AbstractPersistentObject<?,?>)
          Pdo.create(OrgUnit.class).getPersistenceDelegate();
      po.addReferencingClass(2001, "isReferencingOrgUnitByOrgUnitId");  // Message
    });
  }
  ...
  private static class CacheHolder {
    ...
    private static <X extends OrgUnit<X>> PdoCache<X> createCache() {
      PdoCache<X> cache = (PdoCache<X>) Pdo.createCache(OrgUnit.class, true);
  ...
  public void expireCache(long maxSerial) {
    ...
    CacheHolder.CACHE.expire(getSession(), getTableName(), maxSerial);
  }
  ...
  @Override
  public List<T> selectAnyCached(Collection<Long> ids) {
    return filterByClassId(getCache().selectAny(getDomainContext(), ids));
  }
